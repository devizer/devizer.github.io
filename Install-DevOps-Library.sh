...
Install-File-Smarty-Impl() {
  ...
  local fileContent="$2"
  local err=""
  { echo "$fileContent" > "$fileName" 2>/dev/null && chmod +x "$fileName" >/dev/null 2>&1; } || { echo "$fileContent" | $sudo tee "$fileName" 2>/dev/null 1>&2 && $sudo chmod +x "$fileName" >/dev/null 2>&1; } || err=err
  if [[ -z "$err" ]]; then
    echo "OK: $fileName"
  else
    echo "Installer Error: Unable to extract executable script $(basename "$fileName") to $(dirname "$fileName")" >&2
  fi
}
